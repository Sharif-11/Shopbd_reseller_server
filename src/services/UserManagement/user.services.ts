...
class UserManagementServices {
  ...
  async getReferredSellersByLevel({
    ...
    sellers: Array<{
      ...
      address: string | null
      level: number
      referrerName: string | null // Added referrer name
      referrerPhone: string | null // Added referrer phone for reference
      createdAt: Date
    }>
    totalCount: number
    totalPages: number
    ...
    const referredSellers = await prisma.$queryRaw<
      Array<{
        userId: string
        name: string
        phoneNo: string
...
`
    ...
    const levelCountMap = new Map<number, number>()
