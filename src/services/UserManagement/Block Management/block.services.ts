...
import { default as userManagementServices } from '../../UserManagement/user.services'
...
class BlockService {
  ...
  public async createBlockRecordBySystem({
    userPhoneNo,
    tx,
    actions,
  }: {
    ...
    let createdBlockRecord = await tx.block.findFirst({
      where: {
        userPhoneNo,
      },
    })
    if (createdBlockRecord) {
      ...
      createdBlockRecord = await tx.block.create({
        data: {
          userName: user.name,
          userPhoneNo,
          isActive: true,
